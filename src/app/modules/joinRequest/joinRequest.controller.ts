...
const sendJoinRequest = catchAsync(async (req, res) => {
  ...
  sendResponse(res, { success: true, statusCode: 201, message: 'Join request sent', data });
});
...
const respondToRequest = catchAsync(async (req, res) => {
  ...
  const data = await JoinRequestServices.respondToJoinRequest(userId, req.params.id, action);
  sendResponse(res, { success: true, statusCode: 200, message: `Request ${action}ed`, data });
});
...
const getIncomingRequests = catchAsync(async (req, res) => {
  ...
  sendResponse(res, { success: true, statusCode: 200, message: 'Incoming requests fetched', data });
});
...
const getOutgoingRequests = catchAsync(async (req, res) => {
  ...
  sendResponse(res, { success: true, statusCode: 200, message: 'Outgoing requests fetched', data });
});
...
const cancelJoinRequest = catchAsync(async (req, res) => {
  ...
  sendResponse(res, { success: true, statusCode: 200, message: 'Join request cancelled', data: null });
});
